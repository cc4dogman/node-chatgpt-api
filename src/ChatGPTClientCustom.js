...
export default class ChatGPTClientCustom {
  ...
  async getCompletion(input, onProgress, abortController = null) {
    ...
    if (this.options.proxy) {
      ...
      if (proxyURL.protocol === "socks5:") {
        opts.dispatcher = socksDispatcher({
          type: 5,
...
||>Title:`,
    ...
    const titleGenClient = new ChatGPTClientCustom(
      this.apiKey,
  ...
  async sendMessage(message, opts = {}) {
    ...
    const conversationsCacheKey = sceneId
      ? `${conversationId}_${sceneId}`
      : conversationId;
    let conversation = await this.conversationsCache.get(conversationsCacheKey);
    ...
    await this.conversationsCache.set(conversationsCacheKey, conversation);
  ...
  async buildPrompt(messages, parentMessageId, isChatGptModel = false) {
    ...
    if (this.options.assistantPrompt) {
      const assistantPrompt = `${this.startToken}${this.userLabel}:\n${this.options.assistantPrompt}${this.endToken}\n`;
      assistantPayload = {
        role: "user",
    ...
    if (isChatGptModel) {
      ...
      if (assistantPayload) {
        currentTokenCount += this.getTokenCountForMessage(assistantPayload);
      }
    } else {
    ...
    if (isChatGptModel) {
      if (assistantPayload) {
        return [instructionsPayload, assistantPayload, messagePayload];
      }
